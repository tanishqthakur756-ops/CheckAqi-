...
const SEEDS: Seed[] = [
  ...
  // Districts with spatial IDW predicted data (unmonitored by direct CPCB station)
  { id: "la_leh", name: "Leh", state: "Ladakh", lat: 34.1526, lon: 77.5771, baseAqi: 42 },
  { id: "sk_gangtok", name: "Gangtok", state: "Sikkim", lat: 27.3389, lon: 88.6065, baseAqi: 48 },
  { id: "ar_itanagar", name: "Papum Pare (Itanagar)", state: "Arunachal Pradesh", lat: 27.1, lon: 93.62, baseAqi: 52 },
  { id: "ml_shillong", name: "East Khasi Hills (Shillong)", state: "Meghalaya", lat: 25.5788, lon: 91.8933, baseAqi: 58 },
  { id: "mz_aizawl", name: "Aizawl", state: "Mizoram", lat: 23.7271, lon: 92.7176, baseAqi: 45 },
  { id: "nl_kohima", name: "Kohima", state: "Nagaland", lat: 25.6747, lon: 94.11, baseAqi: 62 },
  { id: "tr_agartala", name: "West Tripura (Agartala)", state: "Tripura", lat: 23.8315, lon: 91.2868, baseAqi: 85 },
  { id: "py_puducherry", name: "Puducherry", state: "Puducherry", lat: 11.9416, lon: 79.8083, baseAqi: 82 },
  { id: "ld_kavaratti", name: "Lakshadweep", state: "Lakshadweep", lat: 10.5669, lon: 72.642, baseAqi: 35 },
  { id: "an_portblair", name: "South Andaman (Port Blair)", state: "Andaman and Nicobar", lat: 11.6234, lon: 92.7265, baseAqi: 38 },
];
...
const PREDICTED_IDS = new Set([
  "la_leh", "sk_gangtok", "ar_itanagar", "ml_shillong", 
  "mz_aizawl", "nl_kohima", "tr_agartala", "py_puducherry", 
  "ld_kavaratti", "an_portblair"
]);
...
export const SAMPLE_DISTRICTS: DistrictListItem[] = SEEDS.map((s) => {
  ...
  const isPredicted = PREDICTED_IDS.has(s.id);
  const distKm = isPredicted ? Math.round(45 + seedRng() * 140) : 0;
  const confidence = isPredicted ? Math.round(75 + seedRng() * 18) : 98;
  return {
    district_id: s.id,
    name: s.name,
    state: s.state,
    ...
    coverage_type: isPredicted ? "predicted" : ("station" as const),
...
export function getSampleDistrict(id: string): DistrictDetail | null {
  ...
  return {
    ...
    active_station_count: isPredicted ? 0 : 1 + (hash(s.id) % 3),
    prediction_info: isPredicted
      ? {
          confidence: Math.round(75 + (hash(s.id) % 18)),
          nearest_station_km: Math.round(45 + (hash(s.id) % 120)),
          interpolated_from: ["CPCB regional stations", "Open-Meteo wind field model"],
          methodology: "Inverse Distance Weighting (IDW) + Meteorological Dispersion Factor",
        }
      : undefined,
  };
}
...
export function getSampleHistory(
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    data.push({
      ...
      avg_wind: wind,
      season,
      station_count: isPredicted ? 0 : 1,
      coverage_type: isPredicted ? "predicted" : (i < 7 ? "interpolated" : "station"),
    });
  }
...
export const SAMPLE_SOURCES: SourceInfo[] = [
  {
    source_name: "CPCB (Central Pollution Control Board)",
    ...
    category: "official",
    coverage_description: "Real-time continuous monitoring ground stations across major cities.",
    api_status: "active",
  },
  {
    source_name: "Open-Meteo Atmospheric Model",
    source_url: "https://open-meteo.com/",
    license_note: "CC BY 4.0",
    last_synced_at: "2026-08-31T14:00:00+00:00",
    category: "weather",
    coverage_description: "High-resolution temperature, wind vector, and precipitation grids.",
    api_status: "active",
  },
  {
    source_name: "IMD (India Meteorological Department)",
    source_url: "https://data.gov.in/resources/imd-historical-data",
    license_note: "Open Government License (India)",
    last_synced_at: "2026-08-31T12:00:00+05:30",
    category: "official",
    coverage_description: "Seasonal monsoon calendars and climate normal baseline data.",
    api_status: "active",
  },
  {
    source_name: "IQAir (AirVisual)",
    source_url: "https://www.iqair.com/india",
    license_note: "Commercial / Public Index Comparison",
    last_synced_at: "2026-08-31T16:00:00+05:30",
    category: "popular_website",
    coverage_description: "Popular global air quality network aggregating station & atmospheric model feeds.",
    api_status: "integrated",
  },
  {
    source_name: "WAQI / AQICN (World Air Quality Index)",
    source_url: "https://aqicn.org/map/india/",
    license_note: "Public Access / Non-commercial",
    last_synced_at: "2026-08-31T16:00:00+05:30",
    category: "popular_website",
    coverage_description: "Real-time visual map indexing over 300 Indian monitoring locations.",
    api_status: "integrated",
  },
  {
    source_name: "OpenAQ Database",
    source_url: "https://openaq.org/",
    license_note: "CC BY 4.0 Open Data",
    last_synced_at: "2026-08-31T13:30:00+00:00",
    category: "popular_website",
    coverage_description: "Universal open API for raw pollutant observations (PM2.5, PM10, NO2).",
    api_status: "integrated",
  },
  {
    source_name: "Google Air Quality API",
    source_url: "https://developers.google.com/maps/documentation/air-quality",
    license_note: "Google Maps Platform Service",
    last_synced_at: "2026-08-31T14:00:00+00:00",
    category: "popular_website",
    coverage_description: "1x1 km surface predictions combining sensor arrays, traffic, and meteorological physics.",
    api_status: "estimated",
  },
  {
    source_name: "BreezoMeter (AccuWeather)",
    source_url: "https://breezometer.com/",
    license_note: "AccuWeather Enterprise API",
    last_synced_at: "2026-08-31T14:00:00+00:00",
    category: "popular_website",
    coverage_description: "Hyper-local environmental dispersion models and health advice indices.",
    api_status: "estimated",
  },
];
